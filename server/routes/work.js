...
// DELETE /api/work/photos/delete-by-date-range — Xóa ảnh theo khoảng ngày và chế độ (Admin/Manager)
router.delete('/photos/delete-by-date-range', authenticate, requireManagerOrAdmin, async (req, res) => {
  try {
    const { startDate, endDate, mode } = req.body && req.body.endDate ? req.body : req.query;
    if (!endDate) {
      return res.status(400).json({ error: 'Vui lòng chọn ngày đến cần xóa ảnh (định dạng YYYY-MM-DD).' });
    }

    const deleteMode = mode || 'housekeeping';
    const result = await deletePhotosByDateRange(startDate || null, endDate, deleteMode);
    sendEventToAll({ type: 'WORK_UPDATE', action: 'delete_photos_range', startDate, endDate, mode: deleteMode });
    res.json({
      message: `Đã xóa ${result.deletedCount} tệp ảnh.`,
      deletedCount: result.deletedCount
    });
  } catch (err) {
    console.error('Delete photos by date range error:', err);
    res.status(500).json({ error: err.message || 'Lỗi server khi xóa ảnh.' });
  }
});

// DELETE /api/work/photos/delete-by-staff — Xóa ảnh theo nhân viên (Admin/Manager)
router.delete('/photos/delete-by-staff', authenticate, requireManagerOrAdmin, async (req, res) => {
  try {
    const { staffName, startDate, endDate } = req.body && req.body.staffName ? req.body : req.query;
    if (!staffName) {
      return res.status(400).json({ error: 'Vui lòng chọn nhân viên cần xóa ảnh.' });
    }

    const result = await deletePhotosByStaff(staffName, startDate || null, endDate || null);
    sendEventToAll({ type: 'WORK_UPDATE', action: 'delete_photos_staff', staffName, startDate, endDate });
    res.json({
      message: result.message || `Đã xóa ${result.deletedCount} tệp ảnh của nhân viên "${staffName}".`,
      deletedCount: result.deletedCount
    });
  } catch (err) {
